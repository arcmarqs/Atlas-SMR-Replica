...
use atlas_persistent_log::stateful_logs::divisible_state::{DivisibleStatePersistentLog, initialize_div_persistent_log};
...
impl<S, D, OPM, POPT, LS, STM> SMRPersistentLog<D, OPM, POPT, LS> for DivisibleStatePersistentLog<S, D, OPM, POPT, LS, STM>
    ...
    fn init_log<K, T, POS, PSP, DLPH>(executor: ExecutorHandle<D>, db_path: K) -> Result<Self>
        where K: AsRef<Path>, T: PersistentLogModeTrait,
              ...
              DLPH: DecisionLogPersistenceHelper<D, OPM, POPT, LS> + 'static,
              Self: Sized {
                initialize_div_persistent_log::<S, D, K, T, OPM, POPT, LS, STM, POS, PSP, DLPH>(executor, db_path)
    }
}
